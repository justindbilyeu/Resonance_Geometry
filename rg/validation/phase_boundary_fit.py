...
from rg.sims.meta_flow_min_pair_v2 import run_phase

def find_eta_c_for_lambda(lam, eta_grid, params_base):
    for eta in eta_grid:
        params = dict(params_base); params['eta'] = eta; params['lambda'] = lam
        regime, lam_max = run_phase(params, steps=600, dt=0.01)
        if regime == 2:
            return float(eta)
    return None
...
def main():
    ...
    params_base = {
        ...
        'mi_ema': args.mi_ema, 'omega_anchor': np.zeros(3), 'eta': 0.0, 'lambda': 0.0
    }
    ...
    rows = []
    for lam in lam_grid:
        eta_c = find_eta_c_for_lambda(lam, eta_grid, params_base)
    ...
    if len(pts) >= 2:
        ...
        plt.title("Phase Boundary Fit"); plt.legend()
