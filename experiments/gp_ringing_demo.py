...
# --- tiny, deterministic “coupled oscillator” toy ---
def simulate_coupled(T=200, beta=0.9, alpha=0.08, tau=40.0, seed=0):
    rng = np.random.default_rng(seed)
    x = np.zeros(T, float)
    v = 0.0
    drive = math.sin
    for t in range(1, T):
        force = beta * drive(2*math.pi * t / max(tau,1.0))
...
def analyze_ringing(x: np.ndarray) -> dict:
    ...
    overshoot = float(np.max(x) - x[0]) if len(x) else 0.0
...
def main():
    ...
    for s in range(args.seeds):
        ...
        for _ in range(args.runs):
            x = simulate_coupled(T=args.steps, beta=args.beta, alpha=args.alpha, tau=args.tau, seed=int(rng.integers(0, 2**31-1)))
            m = analyze_ringing(x)
