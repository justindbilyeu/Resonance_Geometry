...
def sweep_alpha(
    ...
    png_path: Union[Path, str] = "docs/analysis/figures/eigenvalue_real_vs_alpha.png",
) -> dict:
    """Run the alpha sweep and persist CSV/JSON/PNG artifacts."""
    p = Params()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--out-png",
        type=Path,
        default=Path("docs/analysis/figures/eigenvalue_real_vs_alpha.png"),
        help="Path to write the generated PNG figure.",
    )
    parser.add_argument(
        "--seed",
        type=int,
...
def main() -> None:
    args = parse_args()
    np.random.default_rng(args.seed)  # Reserved to mirror CLI contract (deterministic here)
    sweep_alpha(
