...
Let
\begin{equation}
  k(\alpha) \equiv \omega_0^2 - K_0 \alpha \cos(\alpha \phi_{\text{eq}})
\end{equation}
denote the effective stiffness. The characteristic polynomial is
\begin{equation}
  \lambda^2 + \gamma \lambda + k(\alpha) = 0,
\end{equation}
with eigenvalues
\begin{equation}
  \lambda_{\pm}(\alpha) = \frac{-\gamma \pm \sqrt{\gamma^2 - 4 k(\alpha)}}{2}.
\end{equation}

Two simple but important facts follow:
\begin{itemize}
  \item The trace is fixed:
  \[
    \mathrm{tr}\,J(\alpha) = \lambda_+(\alpha) + \lambda_-(\alpha) = -\gamma < 0
  \]
  for all $\alpha$. Thus the real parts of any complex eigenpair are locked at $-\gamma/2$.
  \item The determinant is $\det J(\alpha) = k(\alpha)$. Instability can only arise when $k(\alpha)$ changes sign.
\end{itemize}

As long as the discriminant satisfies $\gamma^2 - 4 k(\alpha) < 0$, the eigenvalues form a complex conjugate pair with
\[
\Re \lambda_{\pm}(\alpha) = -\gamma/2 < 0,
\]
corresponding to a stable spiral. When $k(\alpha)$ passes through zero, one eigenvalue crosses $\lambda=0$ and the equilibrium becomes non-hyperbolic ($\lambda_1=0$, $\lambda_2=-\gamma$). For $k(\alpha)<0$, $\det J<0$ and the equilibrium is a saddle with one positive and one negative real eigenvalue.

In particular, because $\mathrm{tr}\,J(\alpha)$ is strictly negative and independent of $\alpha$, this system \emph{cannot} undergo a Hopf bifurcation by varying $\alpha$ alone: a Hopf crossing would require $\mathrm{tr}\,J=0$ with $\det J>0$ at the critical point. Any loss of linear stability here is necessarily via a real eigenvalue crossing associated with the sign change of the effective stiffness $k(\alpha)$.

\paragraph{Retraction note (added 2026-09-02, not by the author).}
The Resonant Transition Point at $\alpha\approx0.35$ asserted in the abstract
and in Section~1 \emph{does not exist}. On the $\phi_{\text{eq}}=0$ branch --
the only equilibrium for $\alpha<\alpha^\star$ -- the effective stiffness
$k(\alpha)=\omega_0^2-K_0\alpha$ is exactly linear, so every smooth quantity of
the system is a function of a linear function of $\alpha$. A sweep of seven such
quantities (Im$\,\lambda$, damping ratio, quality factor, spiral pitch, Fisher
metric $g_{\alpha\alpha}$, metric strain, nonlinear stiffness ratio) over
20{,}001 points finds \emph{zero} sign changes in any second derivative below
$\alpha^\star$: fixed convexity throughout, no extremum, no inflection, nothing
distinguished at $0.35$ or anywhere else. Three separate hypotheses for the
number's origin (a stale $K_0$, an external drive, fixed-time sampling) were
each checked against the repository's full history and each falsified.
The Hopf falsification in this section is unaffected and stands.
See \texttt{RTP\_NULL\_RESULT.md} in this directory for the derivation, the
kill test, and the forensics; \texttt{tests/test\_rtp\_null.py} re-runs the kill
test on every push. The abstract has deliberately not been edited --
retracting a claim is the author's act.
...
Analytically, in the regime where the discriminant $\gamma^2 - 4k(\alpha) < 0$, the eigenvalues form a complex conjugate pair with real part fixed at $-\gamma/2$. The observed "flat band" at $\mathrm{Re}(\lambda)\approx -0.04$ in Figure~\ref{fig:eigs-narrow} is therefore not just a numerical plateau but a direct consequence of the constant damping $\gamma=0.08$: the system remains a strictly stable spiral throughout the RTP region.
...
\subsection{Onset of Linear Instability: Wide and Zoom Sweeps}

Having ruled out a Hopf bifurcation in the RTP region, we extended the parameter range to search for any
linearized instability. A wide sweep from $\alpha = 0.10$ to $1.00$ (Figure~\ref{fig:eigs-wide-zoom}, top panel)
reveals a crossing at $\alpha^\star \approx 0.835$ where one \emph{real} eigenvalue changes sign:

\begin{itemize}
  \item Last stable point: $\alpha = 0.830$, $\lambda_{\text{max}} = -0.040$
  \item First unstable point: $\alpha = 0.840$, $\lambda_{\text{max}} = +0.058$
\end{itemize}

Because $\mathrm{tr}\,J(\alpha) = -\gamma < 0$ is constant, the eigenvalues cannot form an unstable complex
conjugate pair. Instead, the instability arises when the effective stiffness $k(\alpha)$ changes sign,
causing the determinant $\det J = k(\alpha)$ to become negative. This is a \textbf{saddle-type} instability:
one real eigenvalue crosses zero from below while the other remains negative.

A zoom sweep with step size $0.001$ (Figure~\ref{fig:eigs-wide-zoom}, bottom panel) refines the crossing to:

\begin{equation}
  \alpha^\star = 0.833051 \pm 0.000508
\end{equation}

with precision limited by the discretization step. Beyond this point, the system is linearly unstable but
not oscillatory in the neighborhood of equilibrium.
...
\begin{figure}[t]
  ...
  \includesvg[width=\columnwidth]{eigenvalue_real_vs_alpha}
  \caption{%
  \textbf{Saddle-type instability at $\alpha\approx0.833$:} (Top) Wide sweep $\alpha\in[0.10,1.00]$ shows
  a real eigenvalue crossing zero at $\alpha^\star\approx0.835$. (Bottom) Zoom sweep $\alpha\in[0.80,0.86]$
  refines the crossing to $\alpha^\star\approx0.833051\pm0.000508$. Two regimes emerge: an early geometric
  reorganization (RTP) with full stability, and a later saddle-type linear instability when $k(\alpha)$ changes sign.
  }
...
\begin{enumerate}
  \item \textbf{Non-Hopf RTP ($\alpha \approx 0.35$):} Global geometric reorganization with all eigenvalues remaining strictly negative (constant real part $-\gamma/2 \approx -0.04$)
  \item \textbf{Saddle-type instability ($\alpha \approx 0.83$):} Linear instability via real eigenvalue crossing zero when effective stiffness $k(\alpha)$ changes sign
\end{enumerate}

The separation of more than a factor of two in coupling strength demonstrates these are fundamentally different
mechanisms. Crucially, the constant trace $\mathrm{tr}\,J = -\gamma$ throughout the parameter range eliminates
the possibility of a Hopf bifurcation at any $\alpha$. Time-series analysis (available in the repository at
\texttt{results/phase/traces/}) confirms qualitative behavioral changes near $\alpha = 0.35$ despite continued
eigenvalue stability, providing additional evidence for the geometric nature of the RTP.
...
Let $x\in\mathcal{M}$ denote system state on a smooth manifold with metric $g$, and let $F(x;\alpha)$ generate the flow $\dot{x}=F$. Linear stability at an equilibrium $x^\ast(\alpha)$ is governed by the Jacobian $J=\mathrm{D}F(x^\ast;\alpha)$. For our system (\S\ref{sec:model}), the fixed trace $\mathrm{tr}\,J(\alpha) = -\gamma < 0$ for all $\alpha$ eliminates the possibility of a Hopf bifurcation (which would require $\mathrm{tr}\,J=0$ with $\det J>0$ at the critical point). Our data confirm RTP at $\alpha\approx0.35$ with $\max_i \Re\lambda_i(J) \approx -0.04$ (= $-\gamma/2$), far from any linear instability.
...
\end{cases}
\]
for a pre-registered threshold $\tau_S$. This separates \emph{global geometric} reorganization from \emph{local linear} instability and explains the two-regime structure we observe (RTP at $\alpha\!\approx\!0.35$; saddle-type instability near $\alpha\!\approx\!0.833$).
